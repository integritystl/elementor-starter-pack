		...
		<div class="sui-accordion-item-body" v-if="status!=='ignore'">
			<div class="sui-box">
				<div class="sui-box-body">
					...
					<p v-html="text_browser_caching"></p>
	...
	export default {
		...
		computed: {
			...
			hsts_warning_text: function () {
				return this.vsprintf(this.__('Note: Do not include the preload directive by default if you maintain a project that provides HTTPS' +
			...
			text_overview: function () {
				return this.vsprintf(this.__('The HTTP Strict-Transport-Security response header (HSTS) lets a web site tell browsers' +
						' that it should only be accessed using HTTPS, instead of using HTTP.' +
						' This is extremely important for websites that store and process sensitive' +
						' information like ECommerce stores and helps prevent Protocol Downgrade and' +
						' Clickjacking attacks. You can learn more about this security header <a target="_blank" href="%s">here</a>.'), 'https://hstspreload.org/');
			},
			text_browser_caching: function () {
				return this.vsprintf(this.__('Choose when the browser should cache and apply the Strict Transport Security policy for.' +
